...
class FillBooksCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $localeRu = $this->em->getRepository(Locale::class)->findOneByLocale('ru');
        if (!$localeRu) {
            $localeRu = new Locale();
            $localeRu->setLocale('ru')->setName('Русский язык');
            $this->em->persist($localeRu);
            $this->em->flush();
        }
        $localeEn = $this->em->getRepository(Locale::class)->findOneByLocale('en');
        if (!$localeEn) {
            $localeEn = new Locale();
            $localeEn->setLocale('en')->setName('English');
            $this->em->persist($localeEn);
            $this->em->flush();
        }
        ...
        for ($page = 1; $page <= 60; $page++) {
            ...
            foreach ($nodeValues as $key => $nodeValue) {
                ...
                $bookNameRu->setName($nameRu)->setLocale($localeRu);
                $bookNameEn = new BookName();
                $bookNameEn->setName($nameEn)->setLocale($localeEn);
                $book
